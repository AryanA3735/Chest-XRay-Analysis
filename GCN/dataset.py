...
class CXRDataset(Dataset):
    ...
    def __init__(self, root_dir, dataset_type = 'train', transform = None):
        ...
        self.classes = pd.read_csv(self.index_dir, header=None,nrows=1).loc[0, :][1:9].to_numpy()
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Error reading image {img_dir}: {e}")
            return None
        if self.transform:
            image = self.transform(image)
        label = self.label_index.iloc[idx, 1:9].astype('int').to_numpy()
...
class CXRDataset_BBox_only(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_dir = os.path.join(self.image_dir, name)
        image = Image.open(img_dir).convert('L')
        label = self.label_index.loc[self.label_index['FileName']==name].iloc[0, 1:9].astype('int').to_numpy()
        ...
        if self.data_arg:
            ...
            image = F.rotate(image, angle, expand=False, center=None) #, resample=Image.BILINEAR)
            ...
            for i in range(8):
                ...
                bbox_img = F.rotate(bbox_img, angle, expand=False, center=None) #, resample=Image.BILINEAR)
