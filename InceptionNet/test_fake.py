...
def test_model(model):
    ...
    for i in rand_list:
        print('{} output: {}\nlabel: {}\n---------------'.format(i, ['{:.4f}'.format(item) for item in outputList[i]],
                                                                 labelList[i]))
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load("savedModels/net_v1_lr_1e-6_bbox_data_arg.pth"))
    # model=test_model(model)


    # Load the saved model state_dict
    try:
        state_dict = torch.load("savedModels/net_v1_lr_1e-6_bbox_data_arg.pth")
        model.load_state_dict(state_dict)
        print("Model loaded successfully!")
    except Exception as e:
        print("Error updating the model:", str(e))
